                ...
                <aside class="doctor-sidebar" aria-label="Menu del medico">
                    <div class="doctor-profile">
                        <div class="doctor-avatar"
                            style="background: linear-gradient(135deg, <?= e($avc1) ?>, <?= e($avc2) ?>);"><?= e($avInitials) ?>
                        </div>
                        <div class="doctor-profile-text">
                            <p class="doctor-greeting">Dr/a.</p>
                            <p class="doctor-name" title="<?= e($friendlyName) ?>"><?= e($friendlyName) ?></p>
                            <?php if ($specialty): ?>
                                <p class="doctor-specialty"><?= e($specialty) ?></p><?php endif; ?>
                        </div>
                    </div>
                    <nav class="doctor-nav" aria-label="Navegacion del portal del medico">
                        <a href="<?= e(base_url('portal-medico/dashboard.php')) ?>"
                            class="doctor-nav-link <?= $active === 'dashboard' ? 'is-active' : '' ?>"><i
                                data-lucide="layout-dashboard" class="h-4 w-4"></i>Inicio</a>
                        <a href="<?= e(base_url('portal-medico/agenda.php')) ?>"
                            class="doctor-nav-link <?= $active === 'agenda' ? 'is-active' : '' ?>"><i
                                data-lucide="calendar-days" class="h-4 w-4"></i>Mi agenda</a>
                        <a href="<?= e(base_url('portal-medico/pacientes.php')) ?>"
                            class="doctor-nav-link <?= $active === 'pacientes' ? 'is-active' : '' ?>"><i data-lucide="users"
                                class="h-4 w-4"></i>Pacientes</a>
                        <a href="<?= e(base_url('portal-medico/consulta.php')) ?>"
                            class="doctor-nav-link <?= $active === 'consulta' ? 'is-active' : '' ?>"><i
                                data-lucide="stethoscope" class="h-4 w-4"></i>Consulta</a>
                        <a href="<?= e(base_url('portal-medico/disponibilidad.php')) ?>"
                            class="doctor-nav-link <?= $active === 'disponibilidad' ? 'is-active' : '' ?>"><i
                                data-lucide="calendar-clock" class="h-4 w-4"></i>Disponibilidad</a>
                        <a href="<?= e(base_url('portal-medico/analytics.php')) ?>"
                            class="doctor-nav-link <?= $active === 'analytics' ? 'is-active' : '' ?>"><i
                                data-lucide="bar-chart-3" class="h-4 w-4"></i>Analytics</a>
                        <a href="<?= e(base_url('portal-medico/cuenta.php')) ?>"
                            class="doctor-nav-link <?= $active === 'cuenta' ? 'is-active' : '' ?>"><i data-lucide="shield-check"
                                class="h-4 w-4"></i>Mi cuenta</a>
                        <a href="<?= e(base_url('portal-medico/logout.php')) ?>" class="doctor-nav-link doctor-nav-logout"><i
                                data-lucide="log-out" class="h-4 w-4"></i>Cerrar sesion</a>
                    </nav>
