<?php
/**
 * Stream del PDF de un certificado médico emitido por el doctor.
 * Recibe ?id=<certId>. Llama a la API interna con el JWT server-side
 * (que nunca llega al cliente) y devuelve los bytes del PDF.
 */
...
if (!$id) {
    ...
    echo 'Falta el ID del certificado.';
    exit;
}

$res = portal_api_call_binary('GET', '/portal-doctor/me/certificates/' . $id . '.pdf', [], doctor_token());

if (!$res['ok']) {
    $msg = 'No se pudo generar el certificado.';
    ...
    echo '<h1>No se pudo generar el certificado</h1><p>' . htmlspecialchars($msg, ENT_QUOTES, 'UTF-8') . '</p>';
    echo '<p><a href="javascript:history.back()">Volver atrás</a></p></body></html>';
    exit;
}

$filename = $res['filename'] ?: ('certificado_' . $id . '.pdf');
