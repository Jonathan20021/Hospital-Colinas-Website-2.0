/**
 * Portal del Paciente — capa PWA (Hospital General Las Colinas)
 *
 * - Registra el Service Worker (scope /portal/).
...
(function () {
  ...
  var CFG = window.HGLC_PWA || {};
  var LS = {
    install: 'hglcPwaInstallDismissed',
    ios: 'hglcPwaIosHintDismissed',
  };
  ...
  function registerSW() {
    ...
    // updateViaCache:'none' → el script del SW se pide SIEMPRE a la red en cada
    // comprobación, nunca del HTTP cache: jamás se queda con un SW viejo.
  ...
  function showInstall() {
    ...
    c.innerHTML =
      ...
      '<div class="pwa-install-tx"><strong>Instala tu portal</strong>' +
  ...
  // Banner de instrucciones genéricas (navegador sin prompt nativo disponible).
  function showManualHint() {
    if (isStandalone() || document.querySelector('.pwa-install')) return;
    var c = el('div', 'pwa-install');
    c.innerHTML =
      '<img class="pwa-install-ic" src="' + (CFG.icon || '') + '" alt="">' +
      '<div class="pwa-install-tx"><strong>Instala el portal</strong>' +
      '<span>Abre el menú de tu navegador y elige «Instalar app» o «Añadir a inicio».</span></div>';
    var ok = el('button', 'pwa-btn-icon', SVG.close);
    ok.setAttribute('aria-label', 'Entendido');
    ok.addEventListener('click', function () { c.classList.remove('in'); setTimeout(function () { c.remove(); }, 220); });
    c.appendChild(ok);
    getDock().appendChild(c);
    requestAnimationFrame(function () { c.classList.add('in'); });
  }

  // API pública para un botón "Instalar app" explícito en la interfaz.
  window.HGLCPwa = {
