 ...
 * El visor en el navegador llama  /api/imaging-dwr.php/{scope}/{dwpath...}  y este
 * reenvía —en streaming— al proxy interno de JENOFONTE con el JWT del médico (que
 * vive en la sesión server-side). El navegador nunca ve el token ni la IP interna
 * del PACS. Solo GET (lectura); el acceso ya está acotado por el scope-token firmado
 * y por la sesión del médico.
 */
...
if ($pi === '') {
    ...
    $pos = stripos($uriPath, 'imaging-dwr.php');
    if ($pos !== false) $pi = substr($uriPath, $pos + strlen('imaging-dwr.php'));
}
$pi = ltrim($pi, '/');
if ($pi === '' || strpos($pi, '..') !== false) { http_response_code(400); exit; }

$qs     = $_SERVER['QUERY_STRING'] ?? '';
$target = rtrim(portal_api_base(), '/') . '/portal-doctor/imaging/dwr/' . $pi;
if ($qs !== '') $target .= '?' . $qs;
