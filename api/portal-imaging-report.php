<?php
/**
 * Proxy de informes radiológicos (PDF) del PORTAL DEL PACIENTE (mismo origen) para el visor.
 * El navegador llama  /api/portal-imaging-report.php/{scope}/{studyUID}?u=<unq>  y este
 * reenvía —en streaming— al endpoint interno de JENOFONTE con el JWT del PACIENTE (que vive
 * en la sesión server-side). El navegador nunca ve el token, el unq ni la IP interna del RIS.
 * Solo GET; el acceso está acotado por el scope-token firmado (ligado a ESTE paciente) y por
 * la sesión, y el unq se valida en JENOFONTE contra los informes reales del estudio autorizado.
 */
...
if ($pi === '') {
    ...
    $pos = stripos($uriPath, 'portal-imaging-report.php');
    if ($pos !== false) $pi = substr($uriPath, $pos + strlen('portal-imaging-report.php'));
}
$pi = ltrim($pi, '/');
if ($pi === '' || strpos($pi, '..') !== false) { http_response_code(400); exit; }

$qs     = $_SERVER['QUERY_STRING'] ?? '';
$target = rtrim(portal_api_base(), '/') . '/portal/imaging/report/' . $pi;
if ($qs !== '') $target .= '?' . $qs;
