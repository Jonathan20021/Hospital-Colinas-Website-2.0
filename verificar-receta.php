<?php
/**
 * Verificación pública de autenticidad de documentos clínicos (recetas, etc.).
 * Se abre desde el QR impreso en el PDF: /verificar-receta?token=XXXX
...
if (strlen($token) < 16) {
    ...
    $res = portal_api_call('GET', '/portal-doctor/verify/' . $token, []);
    if (!empty($res['ok'])) {
        $data = $res['data'] ?? null;
    } else {
...
<head>
    ...
    <title>Verificación de documento · Hospital General Las Colinas</title>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        body{font-family:'Inter',system-ui,sans-serif;background:
            radial-gradient(1100px 600px at -10% -20%, rgba(79,70,229,.10), transparent 60%),
            radial-gradient(900px 600px at 110% 0%, rgba(20,184,166,.08), transparent 55%), #eef1f7;
            color:#101728;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px}
        .card{background:#fff;border-radius:22px;max-width:460px;width:100%;overflow:hidden;
            box-shadow:0 30px 80px -34px rgba(20,18,60,.45),0 8px 24px -16px rgba(20,18,60,.2)}
        .top{padding:26px 28px 0;text-align:center}
        .top img{height:42px;width:auto}
        .state{padding:24px 28px;text-align:center}
        ...
        .ok .ico{background:#e7f7ee}.ok .ico svg{color:#059669}
        ...
        .ok h1{color:#047857}.bad h1{color:#b91c1c}
    ...
    <div class="card">
        ...
        <?php if ($valid): ?>
            <div class="state ok">
                ...
                <p>Este documento fue emitido por el Hospital General Las Colinas.</p>
            </div>
            <div class="details">
        ...
        <?php else: ?>
            <div class="state bad">
                ...
                <h1>No verificado</h1>
                <p><?= e($error ?: 'No encontramos un documento con este código. Verifica que escaneaste el QR correcto.') ?></p>
            </div>
